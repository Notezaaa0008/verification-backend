    ...
    if (dataLog.length > 0) {
      dataLog.forEach(item => {
        ...
        let calTakeTime = takeTime => {
          let days = Math.floor(takeTime / 3600 / 24);
          let hours = Math.floor((takeTime - days * 3600 * 24) / 3600);
          let minutes = Math.floor((takeTime - days * 3600 * 24 - hours * 3600) / 60);
          let seconds = takeTime - days * 3600 * 24 - hours * 3600 - minutes * 60;
          ...
          performance = {
            ...
            [endDate.getFullYear()]: {
              [monthName[endDate.getMonth()]]: [
                { user: `${item.User.firstName} ${item.User.lastName}`, numberOfVerify: 1 }
              ]
            }
          };
        } else {
          if (Object.keys(performance).includes(`${endDate.getFullYear()}`)) {
            if (Object.keys(performance[endDate.getFullYear()]).includes(monthName[endDate.getMonth()])) {
              let exist = performance[endDate.getFullYear()][monthName[endDate.getMonth()]].filter(
                element => element.user === `${item.User.firstName} ${item.User.lastName}`
              );
              if (exist.length > 0) {
                exist[0].numberOfVerify += 1;
              } else {
                performance[endDate.getFullYear()][monthName[endDate.getMonth()]].push({
                  user: `${item.User.firstName} ${item.User.lastName}`,
                  numberOfVerify: 1
                });
              }
            } else {
              performance[endDate.getFullYear()] = {
                ...performance[endDate.getFullYear()],
                [monthName[endDate.getMonth()]]: [
                  { user: `${item.User.firstName} ${item.User.lastName}`, numberOfVerify: 1 }
                ]
      ...
      res.status(200).json({ performance, timeVerified });
    } else {
